...
| **Trending** | staleness-scheduled (cadence in settings; default weekly; manual refresh) per tracked field — the refresh itself now runs server-side behind `POST /api/skills/trending/refresh` | retrieve recent papers + citation movers (deterministic) → compute metrics (counts, %Δ, weekly series, top movers/venues — deterministic, never LLM-emitted) → qualitative survey (strong, persona-free) | `FieldPanel` per tracked field: deterministic metrics/charts + LLM survey (notable-paper whys, emerging topics, momentum narrative) |
