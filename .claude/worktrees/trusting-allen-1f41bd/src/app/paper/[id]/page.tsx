...
export default function PaperPage() {
  ...
  const [liked, setLiked] = useState(paper?.liked ?? false);
  const [saved, setSaved] = useState(paper?.saved ?? false);
  const [readLater, setReadLater] = useState(paper?.readLater ?? false);
      ...
      <div className="flex-1 overflow-y-auto bg-page-bg">
        <div className="max-w-4xl mx-auto px-8 py-6">
          ...
          <div>
            ...
            {paper.originalTitle && (
              <p className="text-[14px] text-muted-text tracking-body mt-2">
                <span className="text-muted-text">Original: </span>
                {paper.originalTitle}
              </p>
            )}
            {paper.specialty && (
          ...
          {paper.authors && paper.authors.length > 0 && (
            <div className="flex flex-wrap gap-2 mt-4">
              {paper.authors.map((author) => (
                <span
                  key={author}
                  className="px-3 py-1.5 border border-border-warm rounded-pill text-[13px] text-espresso"
                >
                  {author}
                </span>
              ))}
          ...
          <div className="flex items-center gap-2 mt-4">
            ...
            <button
              onClick={() => setReadLater((v) => !v)}
              className="flex items-center gap-1.5 px-4 py-2 rounded-pill text-[13px] transition-colors border border-border-warm text-espresso hover:bg-card-surface"
            >
              <Clock size={14} />
              Read Later
            </button>
            <button className="flex items-center gap-1.5 px-4 py-2 rounded-pill text-[13px] transition-colors border border-border-warm text-espresso hover:bg-card-surface">
              <Share2 size={14} />
              Share
            </button>
          </div>
