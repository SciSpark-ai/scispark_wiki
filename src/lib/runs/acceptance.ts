...
export function summarizeAcceptance(
  ...
  for (const [skill, { applied, reverted }] of bySkill) {
    ...
    const matchedUsage = usageRecords.filter((r) => usageNames?.includes(r.skill))
    const totalCostUsd = !usageNames || matchedUsage.some((r) => r.costUsd === null)
      ? null : matchedUsage.reduce((sum, r) => sum + r.costUsd!, 0)
