    ...
    <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain py-4" aria-label="Report content" onClick={(event) => {
      ...
      {!run ? <p className="text-muted-text">Loading review…</p> : <>
        {version ? <>
          ...
          {!editing && <div className="my-5 flex flex-wrap gap-3 border-y border-border-warm py-3 text-sm"><Button variant="secondary" onClick={beginEdit}>Edit report</Button><a className="self-center text-orange" href={`/api/reviews/${id}/export?version=${version.id}&format=markdown`}>Markdown</a><a className="self-center text-orange" href={`/api/reviews/${id}/export?version=${version.id}&format=bibtex`}>BibTeX</a><Button variant="quiet" disabled={busy} onClick={async () => { if (await act({ action: "knowledge-base", versionId: version.id })) setNotice("Added to your knowledge base. Undo is available in History → Changes; this report stays here.") }}>Add to knowledge base</Button></div>}
