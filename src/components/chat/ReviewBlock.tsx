...
export function ReviewBlock({ id }: { id: string }) {
  ...
  return <section className="mt-4 border-t border-border-warm pt-4" aria-label="Literature review">
    <div className="flex flex-wrap items-center justify-between gap-2">
      ...
      <span className="text-xs text-muted-text">${snapshot.spending.spentUsd.toFixed(3)} of ${run.brief.allowanceUsd.toFixed(2)}</span>
    </div>
    {editing ? <BriefEditor key={run.revision} run={run} busy={busy} onCancel={() => setEditing(false)} onSave={async (action) => { if (await act(action)) setEditing(false) }} /> : <>
      {run.status === "awaiting-approval" && <>
        ...
        <p className="mt-3 text-xs leading-relaxed text-muted-text">{run.brief.sources.join(", ")} · {run.brief.model.model} · thinking enabled</p>
        <p className="mt-1 text-xs leading-relaxed text-muted-text">The allowance is an estimated AI spending limit, not a fixed review price. Source-service charges are separate.</p>
        ...
        {!run.brief.model.rates && <p role="status" className="mt-3 text-sm text-espresso">Add this model&apos;s token prices in Edit brief to enforce your allowance.</p>}
      ...
      <div className="mt-4 flex flex-wrap items-center gap-3">
        {run.status === "awaiting-approval" && <Button disabled={busy || !run.brief.model.rates} onClick={() => void act({ action: "approve", revision: run.revision })}>Start review</Button>}
      ...
      {snapshot.spending.uncertain && !active && <details className="mt-3 text-sm text-espresso"><summary>Resolve uncertain billing</summary><p className="my-2">A prior call may have cost up to ${snapshot.spending.heldUsd.toFixed(3)} without returning a usable result. Retrying can incur another charge.</p><Button disabled={busy} onClick={() => void act({ action: "resume", revision: run.revision, acknowledgeUncertainCharge: true })}>Count that charge and resume</Button></details>}
    </>}
