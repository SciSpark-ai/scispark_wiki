 ...
 * ndjsonSkillRoute), loadSettings(vault), and a Node searchFn — so the client
 * only ever sends the tracked fields. `setSkillTestOverrides` lets tests
 * inject a MockProvider/fake searchFn instead of nodeSearchFn()'s real
 * network calls.
 */
