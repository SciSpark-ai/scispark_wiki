...
import { readRecommendationFeedback, recordRecommendationFeedback, RecommendationFeedbackError } from "@/lib/recommendation/feedback"
...
const Input = z.object({ paperKey: z.string().min(1).max(500), reason: FeedbackReasonSchema,
  note: z.string().trim().max(600).optional(), expectedRevision: z.string().regex(/^[a-f0-9]{64}$/).optional(),
...
export async function GET() {
  ...
  return Response.json({ entries, warning })
}
