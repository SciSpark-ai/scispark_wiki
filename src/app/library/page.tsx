...
export default function LibraryPage() {
  ...
  const actions = usePaperActionsStore((s) => s.actions);

  const filteredPapers = mockPapers.filter((paper: Paper) => {
    const flags = actions[paper.id];
    if (activeTab === "saved") return flags?.saved ?? paper.saved;
    if (activeTab === "liked") return flags?.liked ?? paper.liked;
    if (activeTab === "readLater") return flags?.readLater ?? paper.readLater;
    return false;
  });
        ...
        {tabs.map((tab) => {
          ...
          return (
            <button
              ...
              className={`flex items-center gap-1.5 px-4 py-[7px] text-[13px] font-medium rounded-pill transition-colors ${
                isActive
                  ? "bg-orange text-white"
                  : "bg-card-surface text-muted-text hover:text-espresso"
              }`}
            >
              <Icon
                size={14}
                strokeWidth={1.8}
                className={
                  isActive && tab.id !== "readLater" ? "fill-current" : ""
                }
              />
              {tab.label}
            </button>
